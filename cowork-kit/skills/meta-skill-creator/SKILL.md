...
## Company Context Mode

If `context/SETUP.md` exists, you are operating inside an agentic-os-company
install. Adjust your behaviour as follows:

**Naming:** All skills created for the employee personally must use the prefix
`personal-`. Example: `personal-connector-salesforce`. Add `scope: personal` to
the YAML frontmatter. The folder and `name` field must both use the prefix.

**Scope frontmatter field:** Add to every skill created in company context:
```yaml
scope: personal   # or: company (manager-only, git-tracked)
```

**Connector templates:** Before starting the skill creation interview, check
`.claude/skills/` for `connector-*` folders. If any exist and the employee's
`context/USER.md` lists matching external systems, surface them:
"I see connector templates for {systems}. Would you like to configure a personal
connector for one of those, or build a different skill?"

**Questions to ask instead of brand questions:** In company context, skip questions
about brand voice and audience — those are set company-wide. Instead, focus on:
- The employee's role and daily workflow
- What system or data source the skill should connect to
- What the employee wants to accomplish faster

**On completion:** Tell the employee: "This is a personal skill — it lives only on
your machine. To make it available company-wide, share it with your manager. They
will add it to the company repo and it will auto-sync to everyone on next update."

**Personal skill output path:** `projects/personal-{skill-name}/` (not the standard
`projects/{skill-name}/` — keeps personal outputs separate).
...
- **video**: Optional URL to Simon's walkthrough video for this skill (e.g., `video: "https://youtube.com/..."`). Leave empty string if no video exists yet. Displayed in the "What's New" terminal block after updates and available for users to reference in the frontmatter.
